...
describe('BalanceTrendChart Component', () => {
  ...
  it('sets animation duration to 600ms on chart areas', () => {
    render(<BalanceTrendChart data={mockData} />);
    ...
    expect(expensesArea).toHaveAttribute('data-animation-duration', '600');
  });

  it('sets animation easing to ease-in-out on chart areas', () => {
    render(<BalanceTrendChart data={mockData} />);

    const incomeArea = screen.getByTestId('area-income');
    const expensesArea = screen.getByTestId('area-expenses');

    expect(incomeArea).toHaveAttribute('data-animation-easing', 'ease-in-out');
    expect(expensesArea).toHaveAttribute('data-animation-easing', 'ease-in-out');
  });
